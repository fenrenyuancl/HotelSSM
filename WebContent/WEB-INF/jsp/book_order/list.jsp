            ...
            <tr>
                ...
                <td>
	                <select id="add-accountId" name="accountId" class="easyui-combobox" panelHeight="auto" style="width:268px" data-options="required:true, missingMessage:'请选择客户'">
		            	<c:forEach items="${accountList}" var="account">
		            	<option value="${account.id }" name="${account.name }" id-card="${account.idCard }" phoneNum="${account.phoneNum }">${account.username }</option>
		            	</c:forEach>
	            	</select>	
                </td>
            </tr>
...
function openAdd(){
	...
	*/
	...
	//删除弹窗
	function remove(){
		$.messager.confirm('Confirm','订单不允许删除！',function(r){
		    if (r){
				//alert('ok');
		    	$.messager.alert('确定','我知道了！');
		    }
		}); 
	}
	...
	$("#add-accountId").combobox({
		onSelect:function(data){
			...
			$("#add-phoneNum").val(option.attr('phoneNum'));
		}
	});
	...
	$("#edit-accountId").combobox({
		onSelect:function(data){
			...
			$("#edit-phoneNum").val(option.attr('phoneNum'));
		}
	});
